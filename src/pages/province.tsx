...
const ProvincePage: React.FC<ProvincePageProps> = ({
  ...
  const [regionalAvgTurnout, setRegionalAvgTurnout] = useState<number>(avgTurnout);
    ...
    fetch(`/data/affluenze-puglia.json?t=${timestamp}`, {
      ...
      .then((affluenteData: AffluenteData) => {
        ...
        if (phase >= 0 && affluenteData.enti.ente_p.com_vot[phase]) {
          const regionalTurnout = parseFloat(affluenteData.enti.ente_p.com_vot[phase].perc) || 0;
        ...
        const scrutiniPromises = initialProvinces.map(province => {
          ...
          return fetch(`/data/scrutini-provincia-${provinciaFileName}.json?t=${timestamp}`, {
            cache: 'no-store',
            headers: { 'Cache-Control': 'no-cache', 'Pragma': 'no-cache' }
          })
            .then(r => r.json())
            ...
            .catch(error => {
              console.error(`Error fetching scrutini for ${province.slug}:`, error);
        ...
        Promise.all(scrutiniPromises).then(scrutiniResults => {
          const updatedProvinces = initialProvinces.map((province) => {
            ...
            const provinciaScrutini = scrutiniResults.find(r => r.slug === province.slug);
            const topTwoCandidates = provinciaScrutini?.scrutiniData 
              ? [...provinciaScrutini.scrutiniData.cand].sort((a, b) => b.voti - a.voti).slice(0, 2)
              : [];
            ...
            if (provinciaData && phase >= 0) {
              ...
              return {
                ...
                totalVoters,
                votedCount,
                leadingCandidate: topTwoCandidates[0] ? `${topTwoCandidates[0].nome} ${topTwoCandidates[0].cogn}` : province.leadingCandidate,
                leadingPercentage: topTwoCandidates[0] ? parseFloat(topTwoCandidates[0].perc) || 0 : province.leadingPercentage,
                secondCandidate: topTwoCandidates[1] ? `${topTwoCandidates[1].nome} ${topTwoCandidates[1].cogn}` : province.secondCandidate,
                secondPercentage: topTwoCandidates[1] ? parseFloat(topTwoCandidates[1].perc) || 0 : province.secondPercentage,
              };
            }
            return province;
          });
